...
import {
    From635To640Upgrader,
    PeptideData_v6_3_5,
    PeptideDataSerializer_v6_3_4
} from "@/logic/project/upgraders/From635To640Upgrader";
...
describe("From635To640Upgrader.canUpgrade", () => {
    it("throws when metadata.json is missing", async () => {
        const zip = new JSZip();
        const upgrader = new From635To640Upgrader();
    ...
    it("returns true when project version is older than or equal to 6.3.5", async () => {
        ...
        expect(spyCompare).toHaveBeenCalledWith("6.3.5", "6.3.5");
    });

    it("returns false when project version is newer than 6.3.5", async () => {
...
describe("From635To640Upgrader.upgrade", () => {
    beforeEach(() => {
        vi.restoreAllMocks();
    });
    ...
    it("throws when buffers folder is missing", async () => {
        const metadata = createMetadata("6.3.5", ["a1"]);
        ...
        await expect(upgrader.upgrade(zip)).rejects.toThrow(
            "Failed to find peptide data buffers for analysis 'a1' while upgrading project from 6.3.5 to 6.4.0."
        );
    });
    ...
    it("throws when metadata.json is missing during upgrade", async () => {
        ...
        await expect(upgrader.upgrade(zip)).rejects.toThrow(
            "Failed to find metadata.json while upgrading project from 6.3.5 to 6.4.0."
        );
    });

    it("throws when peptide data buffers for an analysis are missing", async () => {
        const metadata = createMetadata("6.3.5", ["missing"]);
        const zip = await createZipWithMetadata(metadata);
        zip.folder("buffers");
        ...
        await expect(upgrader.upgrade(zip)).rejects.toThrow(
            "Failed to find peptide data buffers for analysis 'missing' while upgrading project from 6.3.5 to 6.4.0."
        );
    });
    ...
    it("upgrades peptide data buffers from V1 to V2, updates project version and analysis configs", async () => {
        ...
        const mapV1 = new ShareableMap<string, PeptideData_v6_3_5>({ serializer: serializerV1 });

        for (const [pep, resp] of Object.entries(responses)) {
            mapV1.set(pep, PeptideData_v6_3_5.createFromPeptideDataResponse(resp));
        }
        ...
        // Metadata version should be updated to 6.4.0
        const updatedMetadata = JSON.parse(await upgraded.file("metadata.json")!.async("string"));
        expect(updatedMetadata.version).toBe("6.4.0");
