 ...
 */
export default class PeptideDataV2 {
    ...
    public static readonly LINEAGE_COUNT_OFFSET: number = PeptideDataV2.LCA_OFFSET + PeptideDataV2.LCA_SIZE;
    ...
    public static readonly LINEAGE_ARRAY_INDEX_OFFSET: number = PeptideDataV2.LINEAGE_COUNT_OFFSET + PeptideDataV2.LINEAGE_COUNT_SIZE;
    ...
    public static readonly FA_ALL_COUNT_OFFSET = PeptideDataV2.LINEAGE_ARRAY_INDEX_OFFSET + PeptideDataV2.LINEAGE_ARRAY_POINTER_SIZE;
    public static readonly FA_EC_COUNT_OFFSET = PeptideDataV2.FA_ALL_COUNT_OFFSET + PeptideDataV2.FA_COUNT_SIZE;
    public static readonly FA_GO_COUNT_OFFSET = PeptideDataV2.FA_EC_COUNT_OFFSET + PeptideDataV2.FA_COUNT_SIZE;
    public static readonly FA_IPR_COUNT_OFFSET = PeptideDataV2.FA_GO_COUNT_OFFSET + PeptideDataV2.FA_COUNT_SIZE;
    ...
    public static readonly FA_EC_INDEX_OFFSET = PeptideDataV2.FA_IPR_COUNT_OFFSET + PeptideDataV2.FA_COUNT_SIZE;
    public static readonly FA_GO_INDEX_OFFSET = PeptideDataV2.FA_EC_INDEX_OFFSET + PeptideDataV2.FA_POINTER_SIZE;
    public static readonly FA_IPR_INDEX_OFFSET = PeptideDataV2.FA_GO_INDEX_OFFSET + PeptideDataV2.FA_POINTER_SIZE;

    // Where does the variable length data portion of the array start?
    public static readonly DATA_START = PeptideDataV2.FA_IPR_INDEX_OFFSET + PeptideDataV2.FA_POINTER_SIZE;
    ...
    public get TAXA_START() {
        let goStart = this.dataView.getUint32(PeptideDataV2.FA_GO_INDEX_OFFSET);
        const goLength = this.dataView.getUint32(goStart);

        let ecStart = this.dataView.getUint32(PeptideDataV2.FA_EC_INDEX_OFFSET);
        const ecLength = this.dataView.getUint32(ecStart);

        let iprStart = this.dataView.getUint32(PeptideDataV2.FA_IPR_INDEX_OFFSET);
        const iprLength = this.dataView.getUint32(iprStart);

        const lineageLength = this.dataView.getUint32(PeptideDataV2.LINEAGE_COUNT_OFFSET);
        const lineageDataLength = PeptideDataV2.LINEAGE_COUNT_SIZE + lineageLength * 4;

        const faDataLength = 12 + goLength * 8 + iprLength * 8 + ecLength * 20;
        return PeptideDataV2.DATA_START + faDataLength + lineageDataLength;
    }
    ...
    public static createFromPeptideDataResponse(response: PeptideDataResponse): PeptideDataV2 {
        ...
        const lineageDataLength = PeptideDataV2.LINEAGE_COUNT_SIZE + response.lineage.length * 4;
        ...
        const taxaStart = PeptideDataV2.DATA_START + faDataLength + lineageDataLength;
        const bufferLength = taxaStart + PeptideDataV2.TAXON_COUNT_SIZE + (response.taxa?.length || 0) * PeptideDataV2.TAXON_SIZE;
        ...
        dataView.setUint32(PeptideDataV2.LINEAGE_ARRAY_INDEX_OFFSET, this.DATA_START);
        ...
        currentPos = taxaStart + PeptideDataV2.TAXON_COUNT_SIZE;
        // Store the actual taxa IDs
        for (const taxon of response.taxa || []) {
            dataView.setUint32(currentPos, taxon);
            currentPos += PeptideDataV2.TAXON_SIZE;
        }

        return new PeptideDataV2(dataView);
    }

    public get faCounts(): { all: number, ec: number, go: number, ipr: number } {
        return {
            all: this.dataView.getUint32(PeptideDataV2.FA_ALL_COUNT_OFFSET),
            ec: this.dataView.getUint32(PeptideDataV2.FA_EC_COUNT_OFFSET),
            go: this.dataView.getUint32(PeptideDataV2.FA_GO_COUNT_OFFSET),
            ipr: this.dataView.getUint32(PeptideDataV2.FA_IPR_COUNT_OFFSET)
        }
    }

    public get lca(): number {
        return this.dataView.getUint32(PeptideDataV2.LCA_OFFSET);
    }

    public get lineage(): (number | null)[] {
        const length = this.dataView.getUint32(PeptideDataV2.LINEAGE_COUNT_OFFSET);
        const lineagePointer = this.dataView.getUint32(PeptideDataV2.LINEAGE_ARRAY_INDEX_OFFSET);
    ...
    public get taxa(): number[] {
        ...
        for (let i = 0; i < taxaLength; i++) {
            output.push(this.dataView.getUint32(this.TAXA_START + PeptideDataV2.TAXON_COUNT_SIZE + i * PeptideDataV2.TAXON_SIZE));
        }
