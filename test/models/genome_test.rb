...
class GenomeTest < ActiveSupport::TestCase
  ...
  test "should not save name changes" do
    ...
    oldValue = genome.name
    genome.name = oldValue + "aa"
    genome.save
    genome.reload
    assert_equal oldValue, genome.name, "Was able to save name changes"
  end

  test "should not save bioproject_id changes" do
    genome = genomes(:genome1)
    oldValue = genome.bioproject_id
    genome.bioproject_id = oldValue + 1
    genome.save
    genome.reload
    assert_equal oldValue, genome.bioproject_id, "Was able to save bioproject_id changes"
  end

  test "should not save insdc_id changes" do
    genome = genomes(:genome1)
    oldValue = genome.insdc_id
    genome.insdc_id = oldValue + "aa"
    genome.save
    genome.reload
    assert_equal oldValue, genome.insdc_id, "Was able to save insdc_id changes"
  end

  test "should not save status changes" do
    genome = genomes(:genome1)
    oldValue = genome.status
    genome.status = oldValue + "aa"
    genome.save
    genome.reload
    assert_equal oldValue, genome.status, "Was able to save status changes"
  end
