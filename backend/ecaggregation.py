...
def getCrossrefData():
	...
	for key, value in pepdir:
		values = set(value)
		for val in values:
			if val in crossdir:
				if seqdir[key][0] not in resultdir:
					resultdir[seqdir[key][0]] = set()
				resultdir[seqdir[key][0]].add(crossdir[val])
...
def aggregation():
	for key, value in resultdir:
		aggrdir={}
		totalperc = float(100)/float(len(value))
		for group in value:
			for val in group:
				if val not in aggrdir:
