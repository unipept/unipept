    ...
    function initTable(showEntries, start) {
        ...
        for (i = start; i < showEntries; i++) {
            if (i < entries.length && entries[i].name !== null) {
                ...
                $('<td class="col-ec"><div class="entry-info">' + ecEntries[i].map(function(element) {return '<a href="http://enzyme.expasy.org/EC/' + element.ec_number_code + '" title="' + String(ec_functions[element.ec_number_code]) + '" target="_blank">' + element.ec_number_code + '</a>'}).join(", ") + '</div></td>').appendTo($row);
                $('<td class="col-go"><div class="entry-info">' + goEntries[i].map(function(element) {return '<a href="http://amigo.geneontology.org/amigo/term/' + element.go_term_code + '" title="' + String(go_functions[element.go_term_code]) + '" target="_blank">' + element.go_term_code + '</a>'}).join(", ") + '</div></td>').appendTo($row);
                $table.append($row[0]);
            }
        }
