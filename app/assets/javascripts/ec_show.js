function init_ec_show(data) {

    // set up the fancy tree
    initecTree(data.tree);
    ...
    function initHelp() {
        ...
        $(".nav-tabs li a span.help").click(function (e) {
            ...
            e.stopPropagation();
            e.preventDefault();
            if ($(this).parent().attr("id") === "ec-tree-tab") {
                title = "EC number tree";
                content = "This interactive tree bundles the complete EC numbers of all UniProt entries whose protein sequence contains " + data.peptide + ". You can click on nodes to expand them, scroll to zoom and drag to move the tree.";
            } else {
                title = "EC number tree";
                content = "This table shows the complete EC numbers of all taxa associated with the UniProt entries whose protein sequence contains " + data.peptide + ". The first column contains the ec number extracted from the UniProt entry, followed by columns representing each part of the ec number ordered from the left to the right.";
            }
    ...
    function setUpImageSave() {
        ...
        $("#save-btn-ec").click(function () {
            ...
            triggerDownloadModal("#ecTree svg", null, "unipept_treeview");
        });
    }
    ...
    function setUpFullScreen() {
        if (fullScreenApi.supportsFullScreen) {
            $("#buttons-second").prepend("<button id='zoom-btn-ec' class='btn btn-default btn-xs btn-animate'><span class='glyphicon glyphicon-resize-full grow'></span> Enter full screen</button>");
    ...
    function initecTree(jsonData) {
        ...
        var svg = d3.select("#ecTree").append("svg")
            .attr("version", "1.1")
